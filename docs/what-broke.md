...
## [2026-09-01] the graduated silence law was documented everywhere and implemented nowhere
Symptom: doc 19 is the north-star document and its section 3 defines the four-band silence ladder that the whole product positioning rests on. The guardian module docstring cited it by name. SCORE_SILENT_KILL sat in constants and silent_kill_floor sat in settings with a monotonicity validator guarding it. Nothing consumed either value: grep found the definition, the validator, and a test of the validator, and no logic anywhere. A scam at 0.96 confidence and a scam at 0.71 produced identical behaviour, and no case carried a band at all.
Root cause: the doctrine was written as vision and the thresholds were added as configuration, but the step that turns a threshold into behaviour was never built. A constant nobody reads is indistinguishable from a constant that does not exist, and the validator gave false assurance because it proved the numbers were ordered, not that they were used.
Fix: compute_silence_band maps every composed verdict onto the ladder from settings, GuardianPackage carries the band, and the escalation path acts on it: a settled scam at or above the floor no longer pages the guardian, while the member who forwarded it still gets warned and a panic press still overrides. The band is persisted on the case row, not only inside the bundle package, because the weekly ledger must not open a bundle to count what stayed silent. False silence joins the metrics beside the false-gate rate, reporting None when a run did not measure it so an unmeasured run can never be read as a clean one.
Prevention: a constant introduced for a documented behaviour ships with the code that consumes it in the same commit, or it does not ship. When a doc names a mechanism, the test that proves the mechanism runs is part of the doc change.
Phase: console and release

## [2026-09-01] the mock model could drive one agent turn and no more
Symptom: writing the first real agent-loop test offline showed the mock model emits a toolUse for tool_specs[0] on every stream call and always stops with tool_use. Any Strands Agent given more than one tool, or asked to finish after calling one, looped forever: call the tool, receive the result, call the same tool again.
Root cause: the double was written for the single-shot structured-output path, where one forced tool call is the whole interaction. That shape was mistaken for the model contract, so the offline harness could never exercise a multi-turn tool loop, and any agent built on tools was untestable without spending on AWS.
Fix: the mock takes an optional script of turns, one consumed per stream call, each either a named tool call or a final text answer. Left unset it behaves exactly as before, which is what keeps every existing single-turn caller unaffected. Running past the end of the script ends the turn rather than raising, so an over-eager loop fails on the assertion that matters instead of on a StopIteration.
Prevention: a test double models the protocol, not the first use case that needed it. When a double cannot express a legal interaction the real dependency allows, that gap is a hole in the test suite whether or not anything is failing yet.
Phase: console and release

## [2026-09-01] the hourly spend breaker was a setting with no mechanism behind it
Symptom: an audit for constants and settings with no consumer found breaker_hourly_call_cap referenced exactly once, at its own definition. The spend module docstring announces "the meter and the breaker", charter principle 7 promises hard caps on Bedrock calls per hour AND per investigation, and only the per-investigation half existed. escalation_floor and engagement_default_opt_in came back from the same audit with the same shape: configuration a reader would reasonably believe was load-bearing, wired to nothing.
Root cause: SpendMeter is constructed fresh inside investigate() for every case, so its budget is per-case by construction. That is correct for what it is and useless as a ceiling: a flood of forwards is a flood of independent budgets, each one comfortably under its own cap while the hour goes unbounded. The per-case breaker being real made the missing one hard to notice, because spend refusals genuinely happened and looked like the system working.
Fix: HourlyBreaker keeps a rolling one-hour window of calls, SpendMeter consults it inside allow() and charges it inside record() so a call can never be accounted to one budget and not the other, and the orchestrator passes the process-wide singleton. The scope limit is stated in the docstring rather than implied: this is process-level state, so under Lambda it bounds one warm container, and a genuine cross-invocation ceiling needs a shared hour-bucketed counter. A conftest fixture resets the singleton between tests, because shared state that survives a test surfaces as a failure in whichever test runs last.
Prevention: the unused-configuration audit runs as a review step, not a lucky grep. Any setting or constant naming a safety behaviour is either consumed by code in the same commit that introduces it or it does not land, and this is now the third defect of exactly that shape in this ledger.
Phase: console and release

## [2026-09-01] a new silence-law test read the host clock and went red after 22:00 IST
Symptom: the suite was green when the silence tests landed in the afternoon and failed that evening on test_unsettled_case_still_reaches_the_guardian, which expected an escalation outcome of sent and got queued. Nothing in the code had changed between the two runs.
Root cause: the test drove handle_telegram_signal without pinning a moment, so the notification service read the real wall clock, found 23:06 IST, and correctly parked the card in the quiet-hours digest. The assertion was about the silence band not suppressing the escalation, and it was written against an outcome that depends on the time of day as well. The file already carried day_epoch and quiet_epoch helpers for exactly this, and the existing quiet-hours tests use them; the new test simply did not.
...
## [2026-09-05] the public console API accepted writes from anyone
Symptom: publishing the console opened its reads to the public, and the write route went out with them. POST /console-api/review reached DynamoDB with no session check at all, validated only the shape of the body, and stamped the row actor "guardian". The household was taken from a caller-supplied x-household header, so a stranger could also choose which household partition to write into. Verified against the live URL: an anonymous POST was rejected for body shape, never for credentials.
Root cause: the split being shipped was "reads public, writes locked", and only the first half was implemented. The handler's own module docstring asserted that every path except /auth validates the session before touching Dynamo, which was false when it was written, so the file documented a guarantee it did not provide and reading it confirmed the wrong thing. validSession already existed and worked; nothing called it on the write path.
Impact: the override ledger is the input to the accuracy counters and the weekly taxonomy. An open write on it means the published precision and false-gate numbers could not be defended, which under principle 4 makes them unpublishable, and it was an unbounded write against a pay-per-request table.
Fix: POST /review returns 401 without a valid session, the household comes from configuration rather than a request header, and /me reports the real session state so the console disables the labelling controls instead of offering a tap that will fail. A second defect surfaced in the same pass: GET /review was never implemented, so the live review counters read 404 and displayed zero labels permanently. It now reduces to the latest label per case rather than counting rows.
Prevention: scripts/check_write_auth.py parses the handler, finds every route guarded on a mutating method, and fails the build if any of them except /auth lacks a validSession call. It was confirmed to fail against the vulnerable version before being accepted. A route that mutates state either proves a session in the same commit or it does not ship.
Phase: console and release

## [2026-09-05] benchmark accuracy was rendered as live measurement
Symptom: the public dashboard showed precision 1.00 with a confidence interval, a 0.0 percent false-gate rate, and p50/p95 latency, sitting in the same tile row as genuinely live counts of screened cases and spend. The numbers were hardcoded in the console API Lambda.
Root cause: the values are real, and match docs/eval-results/full-dev-metrics.json exactly, so nothing was invented. But they measure a 480-case offline dev split, not this household, and nothing on the surface said so. Live accuracy needs labelled outcomes the household has not produced in volume yet. Presenting a benchmark beside live counters with identical styling makes the reader draw a conclusion the data does not support, which is the failure mode principle 4 exists to prevent, and it is worse on a product whose entire claim is measurement honesty.
